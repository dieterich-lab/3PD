 ...
 *
 */
public class EnhancedSuffixArrayIntOptRecomp implements DNASequenceIndex, Serializable{
	...
	private OpenIntIntHashMap lcpmap;
	...
	public EnhancedSuffixArrayIntOptRecomp(String sequence){
	 ...
	 * 
	 * @param maxWordSize the maximum word size
	 * @param includesScanRegion wether this index includes the scan region
	 */
...
//				bucketTab.put(currentString.hashCode(), i);
	...
	/**
	 * Convenience function to set the entry 'SUF_TAB' for index i.
	 * 
	 * @param i the index
	...
	/**
	 * Convenience function to set the entry 'LCP_TABLE' for index 'i'.
	 * 
	 * @param i the index
	 * @param j the value to set
	 */
...
//	}
	...
	protected boolean containsNextIndex(int i){
		assert(i>=0 && i<this.childtab.length);
		return getChildTab(i) < Byte.MAX_VALUE && getChildTab(i) != -1 ? getLcpTab(getChildTab(i)) == getLcpTab(i) : recompChildTabNext(i) != -1; // if no child value can be found during ChildTabNext-recomputation, value '-1' is returned to signal this
	}
...
//	}
	...
	public int[] findMatchPositions(String patter){
		...
		if(!queryFound || interval[0] > interval[1]) elements = 0;
...
//	}
	...
	/**
	 * Returns the substring starting at position 'i' in the index sequence.
	 * 
	 * @param i a position
	 * @return the substring starting at position 'i' in the index sequence
	 */
...
//	}
	...
	/**
	 * Returns the maximum word size this index was created from.
	 * 
	 * @return the maximum word size this index was created from
	 */
	...
	/**
	 * Returns the sequence of this index.
	 * 
	 * @return the sequence of this index
	 */
	...
	/**
	 * Returns true iff this index includes the 3PD scan region.
	 * 
	 * @return true iff this index includes the 3PD scan region
	 */
	...
	/**
	 * Finds the match positions for string 'searchString'.
	 * 
...
//		return result;
	...
	/**
	 * Finds the number of matches of string 'searchString'.
	 * 
	 * @param searchString the string to search
	 * 
	 * @return the number of matches of string 'searchString'
	 */
		...
		if(mode.equals("create")){
			...
			Pattern fastaStart = Pattern.compile(">.*");
				System.out.println("Reading sequence " + file);
				{
					...
					//sequence = "ACAAACATAT";
					testIndex = new EnhancedSuffixArrayIntOptRecomp(sequence);
	...
	//			System.out.println();
				...
				testIndex.serialize(file + ".ESAOptIntRecomp.esaidx");
	 ...
	 * @param maxWordSize tha maximum word size during index creation
	 * @param includesScanRegion whether this index includes the scan region used by 3PD
	 */
