...
public class MultiSeqMemoryMappedESAIndex extends MultiSeqIndex implements Serializable{
	...
	public void createIndex() {
		try{
			...
			while(parser.hasNextContig()){
				...
				currentIndex = new EnhancedSuffixArrayFatOptMemoryMapped(currentContig.getSequence(), currentContig.getID().replaceAll("[ \t]+", "_"), this.fastaFile.getAbsolutePath());
	...
	/**
	 * Deserializes a memory mapped ESA.
	 * 
	 * @param filename the filename to deserialize the memory mapped ESA from
	 * 
	 * @return a memory mapped ESA from a file 
	 */
	...
	public static void main(String[] args){
		SimpleTimer timer = new SimpleTimer();
		timer.startTimer();
		MultiSeqMemoryMappedESAIndex.printStatus = true;
		...
		if(mode.equals("create")){
			...
			index.serialize(new File(file + ".MultiSeqMemoryMappedESAIndex.esaidx"));
