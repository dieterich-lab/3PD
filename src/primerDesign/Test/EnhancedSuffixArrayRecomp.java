 ...
 *
 */
public class EnhancedSuffixArrayRecomp implements DNASequenceIndex{
	...
	public EnhancedSuffixArrayRecomp(){
//		Runtime runtime = Runtime.getRuntime();
//		NumberFormat format = NumberFormat.getInstance();
//		System.gc();
//		System.out.println("At init: " + format.format(runtime.totalMemory()-runtime.freeMemory()));
	}
	
	/**
	 * Creates an enhanced suffix array for the string 'sequence'.
	 * @param sequence the string to construct the enhanced suffix array for
	 */
	public void createIndex(String sequence, int maxWordSize, boolean includesScanRegion){
		...
		//NumberFormat format = NumberFormat.getInstance();
		//System.gc();
		//System.out.println("Before sort: " + format.format(runtime.totalMemory()-runtime.freeMemory()) + "bytes, " + timer.getTimeString());
		quicksortSuffices(0, sequenceLength-1);
		//System.gc();
		//System.out.println("Before LCP Table: " + format.format(runtime.totalMemory()-runtime.freeMemory()) + "bytes, " + timer.getTimeString());
		computeLCPTable();
		//System.gc();
		//System.out.println("Before child table: " + format.format(runtime.totalMemory()-runtime.freeMemory()) + "bytes, " + timer.getTimeString());
		computeChildTable();
		//System.gc();
		//System.out.println("After all: " + format.format(runtime.totalMemory()-runtime.freeMemory()) + "bytes, " + timer.getTimeString());
	}
	...
	private void computeLCPTable(){
		...
		for (int i = 0; i < this.suftab.length; i++) {
			if(i == 0 || i == this.suftab.length-1) setLcpTab(i, 0);
				...
				int end = sequenceLength - Math.max(getSufTab(i), getSufTab(i-1));
				int lcp = 0;
				for (int j = 0; j < end; j++) {
					//if(this.sequence.charAt(getSufTab(i) + j) == this.sequence.charAt(getSufTab(i-1) + j)) incLcpTab(i);
					if(this.sequence[getSufTab(i) + j] == this.sequence[getSufTab(i-1) + j]) lcp++;
					else break;
				}
				setLcpTab(i, lcp);
			}
	...
	protected int getLcpTab(int i){
		assert(i>=0);
		if(this.lcptab[i] < Byte.MAX_VALUE)	return this.lcptab[i] - Byte.MIN_VALUE;
		else return recompLCP(i, i-1);
	}
		...
		if(!interval.isEmptyInterval){
			...
			while(!interval.isEmptyInterval  && c<patter.length() && queryFound){
				if(start != end){
					...
					queryFound = (new String(this.sequence, getSufTab(start)+c , (getSufTab(start)+min)-(getSufTab(start)+c)).equals(patter.substring(c, min)));
					c = min;
					...
					queryFound = (new String(this.sequence, getSufTab(start)+c , (getSufTab(start)+patter.length())-(getSufTab(start)+c)).equals(patter.substring(c, patter.length())));
					break;
				}
			}
		}
	...
	private int recompLCP(int i, int j){
		assert i>= 0 && j>=0 && j<i && i<=sequenceLength && j<=sequenceLength;
		// lcp of first element is 0, lcp between whole sequence and sentinel is 0!!
		if(i==0 || i==sequenceLength) return 0; 
		int result = 0;
		int first = this.suftab[i];
		int second = this.suftab[j];
		for (int k = 0; k < Math.min(sequenceLength-first, sequenceLength-second); k++) {
			//if(this.sequence.charAt(first+k) == this.sequence.charAt(second+k)) result++;
			if(this.sequence[first+k] == this.sequence[second+k]) result++;
			else break;
		}
		return result;
	}
...
//	}
	...
	private int compareSuffix(int a, int b){
		...
		//return getSubstring(getSufTab(a)).compareTo(getSubstring(getSufTab(b)));
		int idxA = getSufTab(a);
		int idxB = getSufTab(b);
		int end = Math.min(sequenceLength-idxA, sequenceLength-idxB);
		for (int i = 0; i < end; i++) {
//			if(this.sequence.charAt(idxA+i) > this.sequence.charAt(idxB+i)) return 1;
//			else if(this.sequence.charAt(idxA+i) < this.sequence.charAt(getSufTab(b)+i)) return -1;
			if(this.sequence[idxA+i] > this.sequence[idxB+i]) return 1;
...
//	}
	...
	public static void main(String[] args) throws IOException{		
		EnhancedSuffixArrayRecomp.timer = new SimpleTimer();
		timer.startTimer();
...
//			System.out.println("Before all: " + format.format(runtime.totalMemory()-runtime.freeMemory()));
			...
			testIndex = new EnhancedSuffixArrayRecomp();
			testIndex.createIndex(sequence, Integer.MAX_VALUE, true);
			System.out.println("EnhancedSuffixArray construction complete - " + timer.getTotalTimestring());
				String query = "ATG";
				...
				System.out.println();
				//((EnhancedSuffixArrayRecomp)testIndex).printArray();
		}
	}
}
