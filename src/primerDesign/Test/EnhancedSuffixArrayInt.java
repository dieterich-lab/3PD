 ...
 *
 */
public class EnhancedSuffixArrayInt implements DNASequenceIndex, Serializable{
	/**
	 * 
	 */
	...
	private int[] suftab;
	private byte[] lcptab;
//	public int[] lcptab_compara;
	private byte[] childtab;
//	public int[] childtab_compara_up;
//	public int[] childtab_compara_down;
//	public int[] childtab_compara_next;
	private IntArrayList lcpmap;
	private HashMap<String, Integer> bucketTab;
	
	public EnhancedSuffixArrayInt(String sequence){
...
//		this.lcptab_compara = new int[sequenceLength+1];
		this.lcpmap = new IntArrayList();
		this.childtab = new byte[sequenceLength+1];
//		this.childtab_compara_up = new int[sequenceLength+1];
//		this.childtab_compara_down = new int[sequenceLength+1];
//		this.childtab_compara_next = new int[sequenceLength+1];
//		Arrays.fill(this.childtab_compara_up, -1);
//		Arrays.fill(this.childtab_compara_down, -1);
//		Arrays.fill(this.childtab_compara_next, -1);
		//this.childmap = new int[0];
		bucketTab = new HashMap<String, Integer>();
...
//				else if(getLcpTab(i) <= getLcpTab(top) && getLcpTab(top) != getLcpTab(lastIndex)) this.childtab_compara_down[top] = lastIndex;
			}
			if(lastIndex != -1){
...
//				this.childtab_compara_next[lastIndex] = i;
	...
	private void computeCombinedChildTable(){
		int lastIndex = -1;
		Stack<Integer> stack = new Stack<Integer>();
		stack.push(0);
		int top;
...
//				this.childtab_compara_up[i] = lastIndex;
			...
			else if(getLcpTab(i) == getLcpTab(stack.peek())){
				lastIndex = stack.pop();
				setChildTab(lastIndex, i);
//				this.childtab_compara_next[lastIndex] = i;
			}
	...
	private void computeBucketTable(){
		...
		int index;
		for(int i=0; i<sequenceLength; i++){
			if(getLcpTab(i) >= d) continue;
			else{
				index = getSufTab(i);
				currentString = getSubstring(index, index+d);
				if(!currentString.equals(lastString) && currentString.length() >= d){
					bucketTab.put(currentString, i);
	...
	private void setChildTab(int i, int j){
		...
		else this.childtab[i] = Byte.MAX_VALUE;
	}
	
	public int recompChildTabUp(int i){
		int candidate = -1;
	...
	public int recompChildTabDown(int i){
		int candidate = -1;
		int min = Integer.MAX_VALUE;
		int reference = getLcpTab(i);
...
//	public int recompChildTabNext(int i){
//		int start = i;
...
//		while(end-start>=0){
//			mean = (start+end)/2;
//			current_lcp = (mean-i>1) ? computeLcp(i,mean-1) : Integer.MAX_VALUE;  // compute smallest lcp between i and precursor of mean
//			if(current_lcp > getLcpTab(mean) && getLcpTab(mean) == getLcpTab(i) && mean > i){
//				candidate = mean;
//				end = mean - 1;
//				if(start == sequenceLength) break;
//			}
//			else if(computeLcp(i, mean) > getLcpTab(i)) start = mean + 1;
//			else if(computeLcp(i, mean) <= getLcpTab(i)) end = mean - 1;
//			else throw new IllegalStateException("Unhandled case!");
//		}
//		return candidate;
//	}
	...
	protected boolean containsDownIndex(int i){
		assert(i>=0 && i<this.childtab.length);
		int value = (getChildTab(i) != -1) ? getChildTab(i) : recompChildTabDown(i);
		return value != -1 && getLcpTab(value) > getLcpTab(i);
	}
	...
	protected boolean containsNextIndex(int i){
		assert(i>=0 && i<this.childtab.length);
		int value = (getChildTab(i) != -1) ? getChildTab(i) : recompChildTabNext(i);
		return value != -1 && getLcpTab(value) == getLcpTab(i) && value > i; // if no child value can be found during ChildTabNext-recomputation, value '-1' is returned to signal this
	}
...
//			while(containsNextIndex(i1)){
			int value;
			while((value = (getChildTab(i1) != -1) ? getChildTab(i1) : recompChildTabNext(i1)) != -1 && getLcpTab(value) == getLcpTab(i1) && value > i1){
				//int i2 = getChildTabNext(i1);
				int i2 = value; //getChildTabNext(i1);
...
//			while(containsNextIndex(k)){
			int value;
			while((value = (getChildTab(k) != -1) ? getChildTab(k) : recompChildTabNext(k)) != -1 && getLcpTab(value) == getLcpTab(k) && value > k){
				result = addInterval(result, k, value-1); //getChildTabNext(k)-1);
				k = value; //getChildTabNext(k);
			}
...
//	}
	...
	public static EnhancedSuffixArrayInt deserialize(String filename){
		EnhancedSuffixArrayInt result = null;
		try{
			ObjectInputStream in = new ObjectInputStream(new GZIPInputStream(new FileInputStream(filename)));
			result = (EnhancedSuffixArrayInt) in.readObject();
		}catch(Exception e){
			e.printStackTrace();
		}
		return result;
	...
	public static void main(String[] args) throws IOException{		
		...
		EnhancedSuffixArrayInt testIndex;
		...
		for(int i=0; i<sequences.length; i++){
			System.out.println("Reading sequence " + sequences[i]);
			{
				...
				//sequence = "ACAAACATAT";
				testIndex = new EnhancedSuffixArrayInt(sequence);
			...
			//testIndex.serialize("EnhancedSuffixArrayInt.ser");
		}
	}
