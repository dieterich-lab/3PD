 ...
 *
 */
public class EnhancedSuffixArrayHashMap implements DNASequenceIndex{
	...
	private HashMap<Int, Int> lcpmap;
	
	public EnhancedSuffixArrayHashMap(){
	...
	public void createIndex(String sequence){
		...
		this.lcpmap = new HashMap<Int, Int>();
	...
	protected int getLcpTab(int i){
		...
		else return ((Int)this.lcpmap.get(new Int(i))).getValue();
	}
	...
	protected void setLcpTab(int i, int j){
		...
		if(j + Byte.MIN_VALUE < Byte.MAX_VALUE) this.lcptab[i] = (byte) (j + Byte.MIN_VALUE);
			...
			this.lcpmap.put(new Int(i), new Int(j));
		}
	}
...
//		else return 0;
	...
	private class Int implements Comparable{
		private int value;
		...
		public int compareTo(Object other) {
			if(this.value < ((Int)other).getValue()) return -1;
			else if(this.value > ((Int)other).getValue()) return 1;
			else return 0;
		}
	...
	public static void main(String[] args) throws IOException{		
		...
		for(int i=0; i<sequences.length; i++){
			...
			testIndex = new EnhancedSuffixArrayHashMap();
